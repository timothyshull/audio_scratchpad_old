...
// NOTE: this corrects a typo in the book on page 203 for the g1 calculation -- WP
void CResonantLPF::calculateMassbergLPFCoeffs(float fCutoffFreq, float fQ) {
    // use same terms as in book:
    float omegaC = 2.0 * pi * fCutoffFreq / (float) m_nSampleRate;
    ...
    float n = pow((float) 2.0 * (float) pi / (fQ * omegaC), (float) 2.0);
    ...
    if (fQ > pow(0.5, 0.5)) {
        ...
        float gr = 2.0 * pow(fQ, (float) 2.0) / pow((float) 4.0 * pow(fQ, (float) 2.0) - (float) 1.0, (float) 0.5);
        float wr = omegaC * pow(1.0 - (1.0 / (2.0 * fQ * fQ)), 0.5);
        ...
        float a = 2.0 - 1 / (fQ * fQ);
        float b = pow((float) 1.0 - ((float) 4.0 * fQ * fQ) / fQ * fQ * fQ * fQ + (float) 4.0 / g1, (float) 0.5);
    ...
    float r = pow(wp / (fQ * omegaC), (float) 2.0);
    ...
    // left channel
    m_LeftLPF.m_f_a0 = alpha0 / a0;
    m_LeftLPF.m_f_a1 = alpha1 / a0;
    m_LeftLPF.m_f_a2 = alpha2 / a0;
    m_LeftLPF.m_f_b1 = beta1 / a0;
    m_LeftLPF.m_f_b2 = beta2 / a0;
...
*/
bool __stdcall CResonantLPF::processAudioFrame(float *pInputBuffer, float *pOutputBuffer, UINT uNumInputChannels,
...
*/
bool __stdcall CResonantLPF::joystickControlChange(float fControlA, float fControlB, float fControlC, float fControlD,
...
// **--0xDEA7--**
    ...
    ui2.m_pUserCookedUINTData = &m_uLPFType;
    ui2.cControlUnits = "";
    ui2.cVariableName = "m_uLPFType";
    ui2.cEnumeratedList = "BZT,MASSBG";
