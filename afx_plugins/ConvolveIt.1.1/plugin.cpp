...
bool __stdcall CPlugIn::processAudioFrame(float *pInputBuffer, float *pOutputBuffer, UINT uNumInputChannels,
                                          UINT uNumOutputChannels) {
    return true;
}


bool __stdcall CPlugIn::processRackAFXAudioBuffer(float *pInputBuffer, float *pOutputBuffer, UINT uNumInputChannels,
                                                  UINT uNumOutputChannels, UINT uBufferSize) {
    return true;
}


bool __stdcall CPlugIn::processVSTAudioBuffer(float **inBuffer, float **outBuffer, UINT uNumChannels,
...
bool __stdcall CPlugIn::joystickControlChange(float fControlA, float fControlB, float fControlC, float fControlD,
...
void CPlugIn::setParameter(UINT index, float value) {
    ...
    switch (pUICtrl->uUserDataType) {
        case intData:
            *(pUICtrl->m_pUserCookedIntData) = calcDisplayVariable(pUICtrl->fUserDisplayDataLoLimit,
                                                                   pUICtrl->fUserDisplayDataHiLimit, value);
            break;

        case floatData:
            *(pUICtrl->m_pUserCookedFloatData) = calcDisplayVariable(pUICtrl->fUserDisplayDataLoLimit,
                                                                     pUICtrl->fUserDisplayDataHiLimit, value);
            break;

        case doubleData:
            *(pUICtrl->m_pUserCookedDoubleData) = calcDisplayVariable(pUICtrl->fUserDisplayDataLoLimit,
                                                                      pUICtrl->fUserDisplayDataHiLimit, value);
            break;

        case UINTData:
            *(pUICtrl->m_pUserCookedUINTData) = calcDisplayVariable(pUICtrl->fUserDisplayDataLoLimit,
                                                                    pUICtrl->fUserDisplayDataHiLimit, value);
            break;
...
float CPlugIn::getParameter(UINT index) {
    ...
    switch (pUICtrl->uUserDataType) {
        case intData: {
            fRawValue = calcSliderVariable(pUICtrl->fUserDisplayDataLoLimit, pUICtrl->fUserDisplayDataHiLimit,
                                           *(pUICtrl->m_pUserCookedIntData));
            break;
        }

        case floatData: {
            fRawValue = calcSliderVariable(pUICtrl->fUserDisplayDataLoLimit, pUICtrl->fUserDisplayDataHiLimit,
                                           *(pUICtrl->m_pUserCookedFloatData));
            break;
        }

        case doubleData: {
            fRawValue = calcSliderVariable(pUICtrl->fUserDisplayDataLoLimit, pUICtrl->fUserDisplayDataHiLimit,
                                           *(pUICtrl->m_pUserCookedDoubleData));
            break;
        }

        case UINTData: {
            fRawValue = calcSliderVariable(pUICtrl->fUserDisplayDataLoLimit, pUICtrl->fUserDisplayDataHiLimit,
                                           *(pUICtrl->m_pUserCookedUINTData));
            break;
        }
