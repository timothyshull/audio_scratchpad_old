...
// basic enums
enum {
    intData, floatData, doubleData, UINTData, nonData
};
enum {
    JS_ONESHOT, JS_LOOP, JS_SUSTAIN, JS_LOOP_BACKANDFORTH
};
...
public:
    ...
    // as you want
    void init(float samplerate, float attack_in_ms, float release_in_ms, bool bAnalogTC, UINT uDetect,
              bool bLogDetector);
...
public:    // Functions
    ...
    void getVectorMixValues(float &fA, float &fB, float &fC, float &fD) {
        fA = m_fA_Mix;
        fB = m_fB_Mix, fC = m_fC_Mix, fD = m_fD_Mix;
    }

    void getVectorACBDMixes(float &fAB, float &fBD) {
        fAB = m_fAC_Mix;
        fBD = m_fBD_Mix;
    }
...
// enum for VU Colors
enum {
    csVU, csRed, csOrange, csYellow, csGreen, csBlue, csViolet
};
