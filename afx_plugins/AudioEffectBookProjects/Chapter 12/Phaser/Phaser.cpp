...
*/
bool __stdcall CPhaser::initialize() {
    // Add your code here

    return true;
}

float CPhaser::calculateAPFCutoffFreq(float fLFOSample, float fMinFreq, float fMaxFreq) {
    return fLFOSample * (fMaxFreq - fMinFreq) + fMinFreq;
}


void CPhaser::calculateFirstOrderAPFCoeffs(float fCutoffFreq, CBiQuad *pBiQuadFilter) {
    // coeff calculation
...
void CPhaser::calculateFirstOrderLeftAPFCoeffs(float fLFOSample) {
    // APF1 - fc -> Bi Quad
    float fCutoffFreq = calculateAPFCutoffFreq(fLFOSample, m_fMinAPF_1_Freq, m_fMaxAPF_1_Freq);
    calculateFirstOrderAPFCoeffs(fCutoffFreq, &m_LeftAPF_1);

    // APF2 - fc -> Bi Quad
    fCutoffFreq = calculateAPFCutoffFreq(fLFOSample, m_fMinAPF_2_Freq, m_fMaxAPF_2_Freq);
    calculateFirstOrderAPFCoeffs(fCutoffFreq, &m_LeftAPF_2);

    // APF3 - fc -> Bi Quad
    fCutoffFreq = calculateAPFCutoffFreq(fLFOSample, m_fMinAPF_3_Freq, m_fMaxAPF_3_Freq);
    calculateFirstOrderAPFCoeffs(fCutoffFreq, &m_LeftAPF_3);

    // APF4 - fc -> Bi Quad
    fCutoffFreq = calculateAPFCutoffFreq(fLFOSample, m_fMinAPF_4_Freq, m_fMaxAPF_4_Freq);
    calculateFirstOrderAPFCoeffs(fCutoffFreq, &m_LeftAPF_4);

    // APF5 - fc -> Bi Quad
    fCutoffFreq = calculateAPFCutoffFreq(fLFOSample, m_fMinAPF_5_Freq, m_fMaxAPF_5_Freq);
    calculateFirstOrderAPFCoeffs(fCutoffFreq, &m_LeftAPF_5);

    // APF6 - fc -> Bi Quad
    fCutoffFreq = calculateAPFCutoffFreq(fLFOSample, m_fMinAPF_6_Freq, m_fMaxAPF_6_Freq);
    calculateFirstOrderAPFCoeffs(fCutoffFreq, &m_LeftAPF_6);


}

void CPhaser::calculateFirstOrderRightAPFCoeffs(float fLFOSample) {
    // APF1 - fc -> Bi Quad
    float fCutoffFreq = calculateAPFCutoffFreq(fLFOSample, m_fMinAPF_1_Freq, m_fMaxAPF_1_Freq);
...
bool __stdcall CPhaser::prepareForPlay() {
    // Add your code here:
    // Add your code here:
    m_LeftAPF_1.flushDelays();
    m_RightAPF_1.flushDelays();

    m_LeftAPF_2.flushDelays();
    m_RightAPF_2.flushDelays();

    m_LeftAPF_3.flushDelays();
    m_RightAPF_3.flushDelays();

    m_LeftAPF_4.flushDelays();
    m_RightAPF_4.flushDelays();

    m_LeftAPF_5.flushDelays();
    m_RightAPF_5.flushDelays();

    m_LeftAPF_6.flushDelays();
    m_RightAPF_6.flushDelays();
...
*/
bool __stdcall CPhaser::processAudioFrame(float *pInputBuffer, float *pOutputBuffer, UINT uNumInputChannels,
    ...
    float fDepth = m_fMod_Depth / 200.0;
    float fFeedback = m_fFeedback / 100.0;
    ...
    float fAPF_1_Out = m_LeftAPF_1.doBiQuad(pInputBuffer[0] + m_fFeedbackLeft * fFeedback);
    float fAPF_2_Out = m_LeftAPF_2.doBiQuad(fAPF_1_Out);
    float fAPF_3_Out = m_LeftAPF_3.doBiQuad(fAPF_2_Out);
    float fAPF_4_Out = m_LeftAPF_4.doBiQuad(fAPF_3_Out);
    float fAPF_5_Out = m_LeftAPF_5.doBiQuad(fAPF_4_Out);
    float fAPF_6_Out = m_LeftAPF_6.doBiQuad(fAPF_5_Out);
    ...
    // do the cascaded APFs
    fAPF_1_Out = m_RightAPF_1.doBiQuad(pInputBuffer[1] + m_fFeedbackRight * fFeedback);
    ...
    if (uNumInputChannels == 2 && uNumOutputChannels == 2)
        pOutputBuffer[1] = fDepth * fAPF_6_Out + (1.0 - fDepth) * pInputBuffer[1];
...
*/
bool __stdcall CPhaser::joystickControlChange(float fControlA, float fControlB, float fControlC, float fControlD,
...
*/
bool __stdcall CPhaser::processRackAFXAudioBuffer(float *pInputBuffer, float *pOutputBuffer,
...
*/
bool __stdcall CPhaser::processVSTAudioBuffer(float **ppInputs, float **ppOutputs,
...
bool __stdcall CPhaser::midiNoteOn(UINT uChannel, UINT uMIDINote, UINT uVelocity) {
    return true;
}

bool __stdcall CPhaser::midiNoteOff(UINT uChannel, UINT uMIDINote, UINT uVelocity, bool bAllNotesOff) {
    return true;
}

// uModValue = 0->127
bool __stdcall CPhaser::midiModWheel(UINT uChannel, UINT uModValue) {
    return true;
}

// nActualPitchBendValue 		= -8192 -> +8191, 0 at center
// fNormalizedPitchBendValue 	= -1.0  -> +1.0,  0 at center
bool __stdcall CPhaser::midiPitchBend(UINT uChannel, int nActualPitchBendValue, float fNormalizedPitchBendValue) {
    return true;
}
...
*/
bool __stdcall CPhaser::midiClock() {

    return true;
}

// any midi message other than note on, note off, pitchbend, mod wheel or clock
bool __stdcall CPhaser::midiMessage(unsigned char cChannel, unsigned char cStatus, unsigned char
...
// **--0xDEA7--**
    ...
    ui1.m_pUserCookedFloatData = &m_fMod_Depth;
    ui1.m_pUserCookedDoubleData = NULL;
    ui1.m_pUserCookedUINTData = NULL;
    ui1.cControlUnits = "%                                                               ";
    ui1.cVariableName = "m_fMod_Depth";
    ...
    ui2.fUserDisplayDataHiLimit = 99.000000;
    ...
    ui2.m_pUserCookedFloatData = &m_fFeedback;
    ui2.m_pUserCookedDoubleData = NULL;
    ui2.m_pUserCookedUINTData = NULL;
    ui2.cControlUnits = "%                                                               ";
    ui2.cVariableName = "m_fFeedback";
    ...
    ui3.m_pUserCookedUINTData = &m_uLFO_Waveform;
    ui3.cControlUnits = "";
    ui3.cVariableName = "m_uLFO_Waveform";
    ...
    ui4.m_pUserCookedUINTData = &m_uLFO_Phase;
    ui4.cControlUnits = "";
    ui4.cVariableName = "m_uLFO_Phase";
    ui4.cEnumeratedList = "NORM,QUAD";
