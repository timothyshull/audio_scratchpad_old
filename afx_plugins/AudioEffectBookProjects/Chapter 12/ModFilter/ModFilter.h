...
class CModFilter : public CPlugIn {
public:
    ...
    float calculateCutoffFreq(float fLFOSample);

    // function to calculate the Q given the LFO sample
    float calculateQ(float fLFOSample);

    // calcualte the left channel coeffs
    void calculateLeftLPFCoeffs(float fCutoffFreq, float fQ);

    // calcualte the right channel coeffs
    ...
    float m_fMod_Depth_fc;
    float m_fModRate_fc;
    UINT m_uLFO_Waveform;
    enum {
        sine, saw, tri, square
    };
    float m_fMod_Depth_Q;
    float m_fModRate_Q;
    UINT m_uLFO_Phase;
    enum {
        NORM, QUAD
