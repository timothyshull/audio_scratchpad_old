...
CBPBSFilterII::CBPBSFilterII() {
    ...
    // Finish initializations here
    m_LeftBiQuad.m_f_a0 = 0.0;
    m_LeftBiQuad.m_f_a1 = 0.0;
    m_LeftBiQuad.m_f_a2 = 0.0;
    m_LeftBiQuad.m_f_b1 = 0.0;
    m_LeftBiQuad.m_f_b2 = 0.0;
    m_LeftBiQuad.flushDelays();

    m_RightBiQuad.m_f_a0 = 0.0;
    m_RightBiQuad.m_f_a1 = 0.0;
    m_RightBiQuad.m_f_a2 = 0.0;
    m_RightBiQuad.m_f_b1 = 0.0;
...
*/
CBPBSFilterII::~CBPBSFilterII(void) {
...
*/
bool __stdcall CBPBSFilterII::initialize() {
    // Add your code here
...
*/
bool __stdcall CBPBSFilterII::prepareForPlay() {
    // Add your code here:
    m_LeftBiQuad.flushDelays();
    m_RightBiQuad.flushDelays();
...
void CBPBSFilterII::calculateCoeffs() {
    float K = tan(pi * m_fFc / (float) m_nSampleRate);
    float a0, a1, a2, b1, b2 = 0;
    float delta = K * K * m_fQ + K + m_fQ;

    if (m_uFilterType == BPF) {
        a0 = K / delta;
        a1 = 0.0;
        a2 = -K / delta;
        b1 = 2.0 * m_fQ * (K * K - 1) / delta;
        b2 = (K * K * m_fQ - K + m_fQ) / delta;
    }
    else if (m_uFilterType == BSF) {
        a0 = m_fQ * (1 + K * K) / delta;
        a1 = 2.0 * m_fQ * (K * K - 1) / delta;
        a2 = m_fQ * (1 + K * K) / delta;
        b1 = 2.0 * m_fQ * (K * K - 1) / delta;
        b2 = (K * K * m_fQ - K + m_fQ) / delta;
    }
...
*/
bool __stdcall CBPBSFilterII::processAudioFrame(float *pInputBuffer, float *pOutputBuffer, UINT uNumInputChannels,
                                                UINT uNumOutputChannels) {
    float xn = pInputBuffer[0];
    pOutputBuffer[0] = m_LeftBiQuad.doBiQuad(xn);
    ...
    if (uNumInputChannels == 2 && uNumOutputChannels == 2) {
        ...
        pOutputBuffer[1] = m_RightBiQuad.doBiQuad(xn);
    }
...
//
bool __stdcall CBPBSFilterII::userInterfaceChange(int nControlIndex) {
    // if anything moves, all coeffs get updated
    calculateCoeffs();
...
*/
bool __stdcall CBPBSFilterII::joystickControlChange(float fControlA, float fControlB, float fControlC, float fControlD,
...
*/
bool __stdcall CBPBSFilterII::processRackAFXAudioBuffer(float *pInputBuffer, float *pOutputBuffer,
...
*/
bool __stdcall CBPBSFilterII::processVSTAudioBuffer(float **inBuffer, float **outBuffer, UINT uNumChannels,
...
bool __stdcall CBPBSFilterII::midiNoteOn(UINT uChannel, UINT uMIDINote, UINT uVelocity) {
    return true;
}

bool __stdcall CBPBSFilterII::midiNoteOff(UINT uChannel, UINT uMIDINote, UINT uVelocity, bool bAllNotesOff) {
    return true;
}

// uModValue = 0->127
bool __stdcall CBPBSFilterII::midiModWheel(UINT uChannel, UINT uModValue) {
    return true;
}

// nActualPitchBendValue 		= -8192 -> +8191, 0 at center
// fNormalizedPitchBendValue 	= -1.0  -> +1.0,  0 at center
bool __stdcall CBPBSFilterII::midiPitchBend(UINT uChannel, int nActualPitchBendValue, float fNormalizedPitchBendValue) {
    return true;
}
...
*/
bool __stdcall CBPBSFilterII::midiClock() {

    return true;
}

// any midi message other than note on, note off, pitchbend, mod wheel or clock
bool __stdcall CBPBSFilterII::midiMessage(unsigned char cChannel, unsigned char cStatus, unsigned char
...
bool __stdcall CBPBSFilterII::initUI() {
    // ADDED BY RACKAFX -- DO NOT EDIT THIS CODE!!! ------------------------------ //
...
// **--0xDEA7--**
    ...
    ui0->bExpSlider = true;
    ui0->fUserDisplayDataLoLimit = 80.000000;
    ...
    ui2->m_pUserCookedUINTData = &m_uFilterType;
    ui2->cControlUnits = "                                                                ";
    ui2->cVariableName = "m_uFilterType";
    ui2->cEnumeratedList = "BPF,BSF";
