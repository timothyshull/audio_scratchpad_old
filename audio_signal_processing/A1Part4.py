...
def downsampleAudio(inputFile, M):
    ...
    # to fix aliasing, etc, add a butter, bessel, or cheby1/2 type lowpass filter from scipy here
    decimated_data = hopSamples(x, M)
    root, ext = os.path.splitext(os.path.basename(inputFile))
    file_dir = os.path.dirname(os.path.abspath(inputFile))
    output_file = '{0}/{1}_downsampled.{2}'.format(file_dir, root, ext)
    wavwrite(decimated_data, fs, output_file)
